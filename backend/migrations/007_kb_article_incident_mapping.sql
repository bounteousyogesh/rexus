-- Migration 007: KB article to incident mapping table
-- Curated relationships between ServiceNow incidents and KB articles.
-- Idempotent: safe to re-run on every startup (IF NOT EXISTS guards; seed uses ON CONFLICT DO NOTHING).
