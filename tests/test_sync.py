...
def test_filter_incidents_by_opened_date_enforces_requested_range():
    ...
    filtered = filter_incidents_by_opened_date(incidents, "2025-09-01", "2025-09-30")
...
def test_upsert_v3_sql_updates_all_columns_on_conflict():
    for col in INCIDENT_ROW_COLUMNS:
        if col == "incident_number":
            continue
        assert f"{col} = EXCLUDED.{col}" in UPSERT_V3_SQL
    assert "updated_at = CURRENT_TIMESTAMP" in UPSERT_V3_SQL
    assert "RETURNING (xmax = 0) AS inserted" in UPSERT_V3_SQL
